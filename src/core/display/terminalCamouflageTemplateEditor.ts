...
} from './camouflageTemplates/customTemplate';
import {
...
import { formatResolvedCamouflageScreen } from './camouflageRender';
import { terminalScreenToPreviewLines } from './camouflagePreview';
...
function buildEditorHtml(webview: Webview, resourceRoot: Uri): string {
  const scriptUri = webview.asWebviewUri(
    Uri.joinPath(resourceRoot, 'terminalCamouflageTemplateEditor.js')
  );
  const styleUri = webview.asWebviewUri(
    Uri.joinPath(resourceRoot, 'terminalCamouflageTemplateEditor.css')
  );
  const csp = [
    "default-src 'none'",
