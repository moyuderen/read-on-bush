...
import type { BookData, EpubProgress, PdfProgress } from './Book';
...
export class PdfBook {
  ...
  constructor(book: BookData, app: ReadBook, extraction: PdfExtraction) {
    ...
    if (
      ...
      this.app.bookList.updatePdfProgress(this.book.id, pdf);
    }
  }
  ...
  private screenFromCurrent(
    ...
    const emptyPage = this.extraction.pages.findIndex(
      (page, pageIndex) => pageIndex > currentPage && page.text.length === 0
    );
