...
- [✨ 特色功能](#-特色功能)
  ...
  - [VS Code 窗口失焦自动隐蔽：窗口切走也不露内容](#vs-code-窗口失焦自动隐蔽窗口切走也不露内容)
...
| VS Code 窗口失焦自动隐蔽 | VS Code 窗口切换到其他应用时自动隐藏伪装阅读内容，降低旁人看到正文的风险 |
...
### VS Code 窗口失焦自动隐蔽：窗口切走也不露内容

这里的“失焦”特指 **VS Code 窗口失去操作系统级窗口焦点**，也就是切换到浏览器、聊天工具或其他应用；不是终端、阅读面板或编辑器之间的内部焦点切换。在 VS Code 窗口内部切换终端、阅读面板和编辑器，不会触发自动隐蔽。

当 VS Code 窗口切换到其他应用时，插件可以自动把终端、阅读面板和状态栏中的正文切换为伪装占位内容。重新回到 VS Code 后，默认保持隐蔽状态，不会擅自显示正文。

- `autoConcealOnFocusLoss` 默认开启，控制 **VS Code 窗口失去焦点** 时是否自动隐蔽
- `autoRevealOnFocus` 默认关闭，开启后 **VS Code 窗口重新获得焦点** 时自动恢复由失焦机制隐藏的内容
- 自动恢复只针对失焦触发的隐蔽，不会覆盖用户手动按 `q` 或 `d` 设置的隐蔽状态
...
安装后点击活动栏的 Read On Bush 图标打开书架视图。点击 **导入书籍** 选择单本文件，或点击 **导入目录** 选择目录批量导入，导入后可点击 **刷新书架** 刷新书架。

### 书架管理

书籍右键菜单：**重命名书籍**、**设置分类** / **清除分类**、**删除书籍**。

书架标题栏：**排序书架**、**分组方式**（不分组 / 按分类 / 按文件目录）、**新建分类**、**刷新书架**、**搜索书籍**。
...
- **设置分类**：右键书籍 → **设置分类**，弹出已有分类列表可直接选择（带数量标注、当前分类高亮），也可底部「创建新分类…」输入新名称或「清除分类」
- **拖拽改分类**：切换到「按分类分组」后，直接把书籍拖到目标分类文件夹或另一本书上即可改分类；拖到「未分类」或空白处则清除分类，支持多选拖拽
- **新建分类**：点击标题栏 **新建分类**，输入名称后多选要加入的书籍
...
1. 点击 **导入书籍** 导入 `.epub` / `.pdf`（或点击 **导入目录** 批量导入），自动提取目录
...
| `autoConcealOnFocusLoss` | `true` | **VS Code 窗口**切换到其他应用时，是否自动将阅读内容切换为伪装占位文案；窗口内部切换终端/面板不触发 |
| `autoRevealOnFocus` | `false` | **VS Code 窗口**从其他应用恢复焦点时，是否自动恢复因失焦而隐藏的内容；手动 `q` / `d` 隐蔽不受影响 |
